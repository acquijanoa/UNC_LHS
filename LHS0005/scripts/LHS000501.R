...
## File: LHS000501.R
## 
## Project: Derive the dataset to obtain the estimates of overweight/obesity in honduras
##
## Description: 
##
...
### Load women data (ENDESA 2019)
women_hon <- read_sav(input_file) ### Women from 15 to 49 years old
...
LHS000501 <- women_hon %>% mutate(
  ...
  rename(WINDEX = windex5,
          ETHNICITY = ethnicity)

## Create the label of the derived variables
LHS000501$REGION <- factor(LHS000501$REGION, 
                           levels = 1:20, 
...
LHS000501$URBAN <- factor(LHS000501$URBAN, levels = 0:1,labels=c("Rural","Urban"))
LHS000501$WOMEN_EDUCATION_C4 <- factor(LHS000501$WOMEN_EDUCATION_C4, levels = 0:3,labels=c("No education",
                                                                                           "Primary",
                                                                                           "Secondary/Highschool",
                                                                                           "Higher education"))
LHS000501$WOMEN_EDUCATION_C3 <- factor(LHS000501$WOMEN_EDUCATION_C3, levels = 0:2,labels=c("No education/Primary",
                                                                                         "Secondary/Highschool","Higher education"))
LHS000501$WINDEX <- factor(LHS000501$WINDEX, levels = 1:5,labels=c("Poorest","Poorer","Middle","Richer","Richest"))

### Save the derived dataset
save(LHS000501, file=output_file)
